...
- Morning startup: the first run within 30 minutes after `quiet_hours_end` forces a full refresh at most once per day. After the forced refresh fires, today's date is persisted to `state/morning_refresh_state.json`; subsequent ticks in the same window see the marker and skip the force-full. A missing, unreadable, or malformed marker is treated as "never" → next tick self-heals. `--force-full-refresh` bypasses the marker check and does not update it.
